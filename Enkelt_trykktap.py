...
if vaeske != 'Egendefinerte kjølevæskeegenskaper':
    ...
    c1,c2 = st.columns(2)
    with c1:
        st.metric("Prandlt-tall", f"{round(Pr,1)}")
    with c2:
        st.metric("Ledningsevne (k)", f"{round(ledningsevne,3)} W/mK")
    c1,c2 = st.columns(2)
    with c1:
        st.metric("Tetthet", f"{round(tetthet)} kg/m^3")
    with c2:
        st.metric("Viskositet", f"{round(viskositet,5)} Pa s")

elif vaeske == 'Egendefinerte kjølevæskeegenskaper':
    c1,c2 = st.columns(2)
    with c1:
        tetthet=st.number_input('Tetthet (kg/m^3)',value=float(1000),step=0.1)
    with c2:
        ledningsevne=st.number_input('Ledningsevne (W/mK)',value=0.5,step=0.01)
    c1,c2 = st.columns(2)
    with c1:
